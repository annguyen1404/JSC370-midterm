---
title: "JSC370 Midterm - A Look at Football Field Goals in "
author: "Chloe Nguyen"
date: "23/02/2022"
...
<!-- DELETE: NOTE ON DATASET RENAMING???? -->

$$\\$$

### The Tools

We'll use dplyr to help merge all datasets and also create new variables. Moreover, we'll use ggplot to create all plots and leaflet for the map. We'll also use broom to tidy all raw outputs and format all tables with kable.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
data <- data %>%
  # filter(specialTeamsPlayType=='Field Goal' | specialTeamsPlayType=='Extra Point')
...
Since the entries of height for this dataset is not standardized (sometimes recorded entirely in inches and sometimes recorded in feet and inches), we will convert all entries of height to cm. We will also convert all entries of weight to kg. Next, we convert all birth dates to Date objects.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# DELETE Check graph for weird values first. See that height is weird. PUT GRAPH OR SOMETHING HERE.
# players %>% filter(nflId=="21213"|nflId=="33469")
# Check with external resources for their actual heights
# players[players$nflId=="21213", "height_cm"] <- 193.04
# players[players$nflId=="33469", "height_cm"] <- 182.88


# DELETE Fix formatting
# players %>% filter(nflId=='25326'|nflId=='27091'|nflId=='38851')
# players[players$nflId=='25326', "birthDate"] <- "1978-03-02"
# players[players$nflId=='27091', "birthDate"] <- "1975-05-29"
# players[players$nflId=='38851', "birthDate"] <- "1990-07-16"
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# DELETE # Number of NAs BEFORE imputing data
# 
# # Columns with at least 1 NA and their type
# data_yes_na <- sort(colnames(data)[colSums(is.na(data)) > 0])
# data_yes_na_type <- sapply(data_yes_na, function(x) ifelse(x %in% numerical_vars, "numerical", "categorical"))
# 
# # Table for columns with NAs
# tibble(`Column` = data_yes_na,
#        `Numerical or Categorical`= data_yes_na_type,
#        `Number of NAs` = sapply(data_yes_na, function(x) sum(is.na(data[[x]])))) %>%
#   kbl(caption = "<center><strong>Columns of Merged Dataset With at Least One NA Observation<center><strong>") %>%
#   kable_styling(bootstrap_options = c("striped", "hover"), full_width = F)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# DELETE # sum(is.na(players$height_cm))
# # sum(is.na(data$kickerId))
# sum(is.na(data$birthDate))
# sum(is.na(data$birthYear))
# sum(is.na(data$height_cm))
# # sum(is.na(data$kickReturnYardage))
# sum(is.na(data$kickweight_kg))
...
<!-- Taking a look at the histogram of kick lengths, we see that the distribution is left-skewed and unimodal. The average kick length is approximately 42 yards. -->

<!-- ```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height=4, fig.align='center'} -->
<!-- # Histogram of Kick Length -->
<!-- data %>% -->
<!--   ggplot( aes(x=kickLength)) + -->
<!--     geom_histogram(fill= "#6C5B7B", alpha=0.6, binwidth=2) + -->
<!--     labs(title = "Histogram of Kick Length") + -->
<!--     xlab("Kick Length") + -->
<!--     ylab("Count") + -->
<!--     theme_minimal() + -->
<!--     theme( -->
...
Similarly, we plot kick length vs. kicker weight for successful and unsuccessul kicks. We also see that for both kick results, the linear regression line is almost completely flat and the points do not follow a particular pattern. Thus, there is no association between these a kicker's weight and their kick length- whether the kick is successful or unsuccessful.
